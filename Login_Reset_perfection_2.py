...
class AuthHandler:
    ...
    @staticmethod
    def verify_user_credentials(unique_code, password):
        try:
            conn = AuthHandler.connect_db()
            if not conn:
                return "Database connection failed"
            cur = conn.cursor()
            cur.execute("SELECT password FROM users WHERE unique_code=%s", (unique_code,))
            result = cur.fetchone()
            cur.close()
            conn.close()
    ...
    @staticmethod
    def reset_user_password(unique_code, new_password):
        try:
            conn = AuthHandler.connect_db()
            if not conn:
                return "Database connection failed"
            cur = conn.cursor()
            cur.execute("SELECT id FROM users WHERE unique_code=%s", (unique_code,))
            if cur.fetchone() is None:
                return "Unique Code not found"
            cur.execute("UPDATE users SET password=%s WHERE unique_code=%s", (new_password, unique_code))
            conn.commit()
            cur.close()
            conn.close()
