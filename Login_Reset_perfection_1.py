...
class ForgotPasswordUI(QWidget):
    ...
    def reset_password(self):
        ...
        if strength == "Weak":
            ...
            return
        try:
            conn = mysql.connector.connect(host="localhost", user="root", password="", database="imhotep_db")
            cur = conn.cursor()
            cur.execute("SELECT id FROM users WHERE unique_code=%s", (code,))
            if cur.fetchone() is None:
                self.error_label.setText("Unique Code not found.")
            else:
                cur.execute("UPDATE users SET password=%s WHERE unique_code=%s", (new, code))
                conn.commit()
                self.error_label.setStyleSheet("color:green;")
                self.error_label.setText("Password successfully updated!")
            cur.close()
            conn.close()
        except db_errors.Error as e:
            self.error_label.setText("Database Error: " + str(e))
...
class LoginUI(QWidget):
    ...
    def handle_login(self):
        ...
        try:
            conn = mysql.connector.connect(host="localhost", user="root", password="", database="imhotep_db")
            cur = conn.cursor()
            cur.execute("SELECT password FROM users WHERE unique_code=%s", (user,))
            res = cur.fetchone()
            if res and res[0] == pwd:
                self.error_label.setText("")  # No pop-up on success
            else:
